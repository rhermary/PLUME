...
from models.backbones import ResNet50Backbone, VGG16Backbone, ConvNeXtBackbone
from metrics import (
    MeasureAUC,
    VAL_AUC,
    MeasureAccuracy,
...
def choose_model(backbone: BaseBackbone) -> Type[PLAD]:
    ...
    """
    if isinstance(backbone, (ResNet50Backbone, VGG16Backbone, ConvNeXtBackbone)):
        return AdaptativeFeatureRCSPLAD
    
    if getattr(backbone, "dense_adaptor", None) is None:
        # return FeaturePLAD
        raise NotImplementedError("Legacy Code")

    return FeatureRCSPLAD


def train(args: argparse.Namespace, run: ActiveRun) -> None:
    """Train AD network with pretrained backbone.
    ...
    end_logs(
        ...
        model_checkpoint,
        check_trace=True,
        # log_sample=args.features_dir is None,
        log_sample=False,
    )
